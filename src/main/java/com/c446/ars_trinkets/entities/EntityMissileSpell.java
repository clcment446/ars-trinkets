package com.c446.ars_trinkets.entities;
...
public class EntityMissileSpell extends EntityProjectileSpell {
    ...
    public EntityMissileSpell(Level world, SpellResolver resolver) {
        super(ModRegistry.ENTITY_MISSILE.get(), world, resolver);
...
//                BlockRegistry.PORTAL_BLOCK.onProjectileHit(this.level(), this.level().getBlockState(new BlockPos(
//                        ((HitResult)((BlockHitResult) raytraceresult).getBlockPos()))), (BlockHitResult)raytraceresult, this);
//            }
    ...
    protected void ActivateSpellAtPos(Vec3 pos) {
        if (!this.level().isClientSide() && this.spellResolver != null) {
            ...
            if (!foundEntity) {
                ...
                Vec3i vecNew = new Vec3i((int) vector3d2.x, (int) vector3d2.y, (int) vector3d2.z);
                this.spellResolver.onResolveEffect(this.level(), new BlockHitResult(vector3d2, Direction.getNearest(dist.x, dist.y, dist.z), new BlockPos(vecNew), true));
            }
        }
    }
    ...
    protected void ExplodeMissile() {
        ...
        Networking.sendToNearby(this.level(), new BlockPos(this.blockPosition()), new PacketANEffect(EffectType.BURST, new BlockPos(this.blockPosition()), this.getParticleColorWrapper().toParticleColor()));
    }
