...
import static com.c446.ars_trinkets.registry.CurioConfigManager.*;

public class GenericMonocle extends RegularItems implements ICurioItem, IManaEquipment {

    protected int level;
    int regen;
    int boost;

    public GenericMonocle(Properties p, int level) {
        super(p);
        this.level = level;
    }
    public GenericMonocle(Properties p, boolean showEnch, int level) {
        super(p, showEnch);
        this.level = level;
    }
    public HashMap<Attribute, Long> createAttributeMap() {
        if (Config.SERVER_CONFIG.isLoaded()) {
            HashMap<Attribute, ForgeConfigSpec.IntValue> map = CurioConfigManager.monocle_maps.get(level);
            return Util.fromConfigToLong(map);
        }
        else {
            HashMap<Attribute, Long> map = new HashMap<>();
            map.put(PerkAttributes.MAX_MANA.get(), (long) defaultMonocleMana[level]);
            map.put(PerkAttributes.MANA_REGEN_BONUS.get(), (long) defaultMonocleRegen[level]);
            map.put(com.c446.ars_trinkets.perks.PerkAttributes.SPELL_DAMAGE_PCT.get(), (long) defaultMonocleDamage[level]);
            return map;
        }
    }
