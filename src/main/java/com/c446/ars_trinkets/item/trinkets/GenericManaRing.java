...
public class GenericManaRing extends RegularItems implements ICurioItem, IManaEquipment {
    ...
    public GenericManaRing(Properties p, int level) {
        super(p);
        this.level = level;
    }
    public GenericManaRing(Properties p, boolean showEnch, int level) {
        super(p, showEnch);
        this.level = level;
    }
    public HashMap<Attribute, Long> createAttributeMap() {
        if (Config.SERVER_CONFIG.isLoaded()) {
            HashMap<Attribute, ForgeConfigSpec.IntValue> map = CurioConfigManager.ring_maps.get(level);
